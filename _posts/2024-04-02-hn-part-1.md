...
---
*UPDATE, 2024-04-09*: I've found it that I didn't filter out that `dead` and `deleted` stories and comments from my analysis. I've updated the analysis accordingly. While there are changes in the numbers and figures, the patterns and observations stand. I've also removed one table related to the second analysis (HN as a competitive platform): 1) it was confusing, 2) it didn't add new information, 3) and with the new data, the categorization bins needed rethinking.
...
An analysis of the behavior of users who post stories on Hacker News reveals that, typically, users cease sharing stories about one year after they begin. This observation is supported by the data visualization below.
